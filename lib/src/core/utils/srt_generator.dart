import 'dart:io';
import 'package:path_provider/path_provider.dart';
import '../models/geo_data_model.dart';

class SrtGenerator {
  static Future<String> generateSrtFile(
    List<GeoData> dataFrames, {
    required String videoFilePath,
  }) async {
    // Construir nombre del SRT
    // Si tu video es /Videos/VID_12345.mp4 => lo convertimos a .srt
    String srtPath = videoFilePath.replaceAll('.mp4', '.srt');
    final buffer = StringBuffer();

    for (int i = 0; i < dataFrames.length; i++) {
      final geo = dataFrames[i];
      int index = i + 1;
      
      // Calculamos la marca de tiempo para SRT
      // Formato: hh:mm:ss,mmm
      // A modo simple: usaremos la diferencia a partir del primer frame
      final startMs = dataFrames
          .take(i)
          .fold<int>(0, (acc, d) => acc + d.diffTimeMs); // sumatorio diffs previos
      final endMs = startMs + geo.diffTimeMs;

      String startStr = _formatSrtTime(startMs);
      String endStr = _formatSrtTime(endMs);

      buffer.writeln('$index');
      buffer.writeln('$startStr --> $endStr');
      buffer.writeln(
          '<font size="28">FrameCnt: ${geo.frameCount}, DiffTime: ${geo.diffTimeMs}ms');
      buffer.writeln(
          '${geo.timestamp.toIso8601String().replaceAll('T', ' ').substring(0, 23)}');
      buffer.writeln(
          '[iso: 100] [shutter: 1/3090.80] [fnum: 2.8] [ev: 0] [color_md: default] [ae_meter_md: 1] [focal_len: 24.00] [dzoom_ratio: 1.00], '
          '[latitude: ${geo.latitude}] [longitude: ${geo.longitude}] '
          '[rel_alt: ${geo.relAltitude} abs_alt: ${geo.absAltitude}] '
          '[gb_yaw: ${geo.yaw} gb_pitch: ${geo.pitch} gb_roll: ${geo.roll}] </font>\n');
    }

    final file = File(srtPath);
    await file.writeAsString(buffer.toString());
    return srtPath;
  }

  // Convierte milisegundos a formato hh:mm:ss,mmm
  static String _formatSrtTime(int milliseconds) {
    final duration = Duration(milliseconds: milliseconds);
    final hours = duration.inHours.toString().padLeft(2, '0');
    final minutes =
        (duration.inMinutes % 60).toString().padLeft(2, '0');
    final seconds =
        (duration.inSeconds % 60).toString().padLeft(2, '0');
    final millis =
        (duration.inMilliseconds % 1000).toString().padLeft(3, '0');
    return '$hours:$minutes:$seconds,$millis';
  }
}
